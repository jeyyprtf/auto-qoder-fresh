...
def extract_otp(html: str | None) -> str | None:
    ...
    # Method 4: plain text / stripped HTML — look for "code is XXXXXX" or "OTP: XXXXXX"
    text_only = re.sub(r"<[^>]+>", " ", html)
    text_only = re.sub(r"\s+", " ", text_only)
    kw = re.search(r"(?:code|otp|verification|kode|otp|is)\s*:?\s*(\d{6})", text_only, re.IGNORECASE)
    if kw:
        return kw.group(1)
    # Method 5: last resort — any standalone 6-digit number in plain text
    plain_codes = re.findall(r"(?<!\w)(\d{6})(?!\w)", text_only)
