...
def main() -> None:
    ...
    if len(sys.argv) > 1:
        ...
        if sub == "claim":
            from .auth.cli_login import claim_with_cli
            ...
            while i < len(sys.argv):
                if sys.argv[i] == "--cli-dir" and i + 1 < len(sys.argv):
                    cli_dir = sys.argv[i + 1]
                    i += 2
                elif sys.argv[i] == "--no-spoof":
                    spoof = False
                    i += 1
                    ...
                    i += 1
            claim_with_cli(config_dir=cli_dir, spoof=spoof, open_browser=open_browser)
            return
        ...
        if sub == "spoof":
            ...
            cli_dir = None
            if len(sys.argv) > 2 and sys.argv[2] == "--cli-dir" and len(sys.argv) > 3:
                cli_dir = sys.argv[3]
            spoof_machine(cli_dir=cli_dir, include_cli=True, include_desktop=True)
            return
    ...
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  claim            Spoof + qodercli login to claim Pro trial\n"
            "  spoof            Reset machine ID & telemetry for fresh identity\n"
            "  vm-check         Check if this machine looks like a VM\n"
            "\n"
            "examples:\n"
            ...
            "  qoder-autopilot spoof\n"
            "  qoder-autopilot vm-check\n"
        ),
