...
async def register_and_verify(
    ...
    try:
        ...
        await asyncio.sleep(2)  # let checkbox animation finish
        ...
        for attempt in range(5):
            ...
            if pw_visible:
                ...
                log(f"   ⚠️ Step 2 not visible yet (attempt {attempt + 1}/5)")
                if attempt == 4:
        ...
        try:
            ...
            await asyncio.sleep(2)  # wait for field to settle
            ...
            if not pw_filled:
                ...
                return False
            await asyncio.sleep(2)
            # Also try clicking via JS after JS fill
            await page.evaluate("""() => {
                const btn = document.querySelector('button[type="submit"], .ant-btn-primary');
                if (btn) { btn.scrollIntoView({block: 'center'}); setTimeout(() => btn.click(), 300); }
            }""")
            await asyncio.sleep(3)

        await asyncio.sleep(5)  # wait for page transition after submit
